...
func _build_ui():
	...
	add_child(
		_make_line_edit(
			"Description", quest_step.description, func(val): quest_step.description = val
		)
	)
	add_child(_make_int_field("Goal", quest_step.goal, func(val): quest_step.goal = val))
	add_child(
		_make_array_editor(
			"Prerequisites", quest_step.prerequisites, func(val): quest_step.prerequisites = val
		)
	)
	add_child(
		_make_line_edit(
			"Play Dialogue", quest_step.play_dialogue, func(val): quest_step.play_dialogue = val
		)
	)
	add_child(
		_make_option_menu(
			"Marker ID",
			quest_step.marker_id,
			func(val): quest_step.marker_id = val,
			[
				"trin",
				"mom",
				"home_entrance",
...
func _add_signal_objective_fields(obj: SignalQuestStep):
	add_child(
		_make_option_menu(
			"Autoload",
			obj.autoload_name,
			func(val): obj.autoload_name = val,
...
func _make_array_editor(label_text: String, initial_array: Array, _setter: Callable) -> Control:
...
func _make_option_menu(
	label_text: String, selected_value: String, setter: Callable, options: Array[String]
) -> Control:
	var hbox = HBoxContainer.new()
	hbox.name = label_text + "_hbox"
	var lbl = Label.new()
